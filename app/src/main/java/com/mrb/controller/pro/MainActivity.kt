...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private fun addCustomButton(def: CustomBtn, x: Float, y: Float, w: Int = -1, h: Int = -1) {
        ...
        val dw = if (w > 0) w else when {
            ...
            def.id == "touchpad" -> 240  // Touchpad by default bada aur choras hoga
            def.id == "tilt_tog" -> 130
            def.id == "kb_space" -> 160
        ...
        val dh = if (h > 0) h else when {
            def.id == "left_joy" || def.id == "right_joy" -> 160
            def.id == "touchpad" -> 200  // Height thodi kam taaki rectangle/square lage
            def.id == "tilt_tog" -> 50
            def.id.startsWith("kb_") -> 60
            else -> 100
        }
    ...
    private fun buildView(def: CustomBtn, pb: PlacedCustomBtn,
        ...
        when {
            def.id == "touchpad"                           -> buildTouchpad(pb, container, inEditMode) // Naya function call
    ...
    private fun buildTouchpad(pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        // UI Design for Touchpad
        container.background = android.graphics.drawable.GradientDrawable().apply {
            setColor(Color.argb(80, 50, 50, 50)) // Semi-transparent black
            setStroke(2, Color.argb(150, 200, 200, 200)) // White-ish border
            cornerRadius = 24f
        }
        ...
        if (!inEditMode) {
            var startX = 0f
            var startY = 0f
            val maxDrag = 150f // Kitne pixels drag karne par speed full ho jayegi (Sensitivity)
        ...
        resizeBtn.setOnTouchListener { _, e ->
            when (e.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    val nw = (rsW + d).coerceIn(40, 600) // Max width increased for touchpad
                    val nh = (rsH + d / 2).coerceIn(40, 600) // Max height increased for touchpad
