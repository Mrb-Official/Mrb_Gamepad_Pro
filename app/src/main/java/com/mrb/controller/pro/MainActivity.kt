...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private fun initUI() {
        ...
        // CHANGE 1: Make overlayFrame non-clickable so it doesn't block gas/brake touches
    ...
    private fun setupDefaultViews() {
        listOf("lay_brake","lay_gas","lay_gear_up","lay_gear_down",
    ...
    private fun showPremiumPopup() {
        ...
        btnAdRow.addView(ImageView(this).apply {
    ...
    private fun toggleEditMode() {
        ...
        if (editMode) {
            ...
            hideEditBar()
            enableDefaultDragging(false)
            // CHANGE 3: Re‑ensure overlay does not block touches after exiting edit mode
            overlayFrame.isClickable = false
            placedCustomBtns.toList().forEach { pb ->
                val v   = placedCustomViews[pb.id] ?: return@forEach
                val def = customButtons.find { it.id == pb.id } ?: return@forEach
    ...
    @SuppressLint("ClickableViewAccessibility")
    private fun enableDefaultDragging(enable: Boolean) {
            ...
            container.setOnTouchListener { _, e ->
                when (e.action) {
                    MotionEvent.ACTION_DOWN -> {
                        lastX = e.x; lastY = e.y; first = true
                    ...
                    MotionEvent.ACTION_MOVE -> {
                        ...
                        first = false; lastX = e.x; lastY = e.y
    ...
    // CHANGE 2: Completely fixed sendReport() with gas/brake as 0–255 and correct axis order
    ...
    private fun sendReport() {
        try {
            ...
            // Gas & Brake as unsigned 0–255 (0xFF = fully pressed)
            ...
            hid.sendReport(device, 1, byteArrayOf(
                b1.toByte(), b2.toByte(),
                leftJoyX, leftJoyY,   // LX, LY
                rightJoyX, rightJoyY, // Z, Rz (touchpad)
                brakeVal, gasVal      // Rx, Ry (brake, gas)
            ))
        } catch (e: Exception) {
            e.printStackTrace()
        }
