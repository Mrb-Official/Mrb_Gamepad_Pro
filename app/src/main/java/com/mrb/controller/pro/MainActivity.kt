...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    // ── Step 2: Old hardcoded boolean variables removed ──────────────────────
    // gasOn, brakeOn, gearUp, gearDown, btnA, btnB, btnX, btnY,
    // dpadUp, dpadDown, dpadLeft, dpadRight  ← all deleted.
    // HID logic now reads directly from customBtnStates map.
    ...
    // ── Step 3: customButtons list — migrated buttons added ──────────────────
    private val customButtons = listOf(
        // ── Formerly hardcoded XML buttons (now dynamic) ──────────────────
        ...
        CustomBtn("dpad_up",    "D▲",      R.drawable.ic_btn_custom,     0xFFCCCCCC.toInt()),
        CustomBtn("dpad_down",  "D▼",      R.drawable.ic_btn_custom,     0xFFCCCCCC.toInt()),
        CustomBtn("dpad_left",  "D◀",      R.drawable.ic_btn_custom,     0xFFCCCCCC.toInt()),
        CustomBtn("dpad_right", "D▶",      R.drawable.ic_btn_custom,     0xFFCCCCCC.toInt()),
        // ── Original custom buttons ───────────────────────────────────────
    ...
    private fun initUI() {
        ...
        // ── Step 2: All old setupTouch() calls for hardcoded buttons removed ──
        // The buttons (gas, brake, gear_up, gear_down, btn_a/b/x/y, dpad_*) are
        // now placed dynamically via the customButtons system instead.
    ...
    private fun buildKbKey(def: CustomBtn, pb: PlacedCustomBtn,
        ...
        if (!inEditMode) {
            ...
            container.setOnTouchListener { _, e ->
                ...
                if (pressed || released) {
                    // kb_up/left/down/right drive the dpad state via customBtnStates
    ...
    // ── Step 4: sendReport() — all HID logic reads from customBtnStates ───────
    ...
    private fun sendReport() {
        try {
            ...
            // ── Face buttons (formerly btnA/B/X/Y) ───────────────────────────
            ...
            // ── Gears now map to shoulder buttons (formerly gearDown/gearUp) ─
            if (customBtnStates["gear_down"] == true) b1 = b1 or (1 shl 2)  // LB
            if (customBtnStates["gear_up"]   == true) b1 = b1 or (1 shl 5)  // RB

            // ── All remaining customButtons contribute their bit assignments ──
            ...
            // ── D-Pad hat value — reads from customBtnStates ─────────────────
            // kb_up/down/left/right also contribute to the D-pad via customBtnStates
            ...
            // ── Triggers (L2 / R2) — read from customBtnStates ───────────────
