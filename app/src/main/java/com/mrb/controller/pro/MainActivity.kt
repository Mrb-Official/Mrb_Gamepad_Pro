...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private fun setupDefaultViews() {
        listOf(
            "lay_brake", "lay_gas", "lay_gear_up", "lay_gear_down",
    ...
    private fun showEditBar() {
        val bar = LinearLayout(this).apply {
            ...
            gravity = Gravity.CENTER_VERTICAL
            layoutParams = FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT, 52
        ...
        for (btn in customButtons) {
            ...
            chip.addView(TextView(this).apply {
                ...
                layoutParams = FrameLayout.LayoutParams(
                    FrameLayout.LayoutParams.MATCH_PARENT,
                    FrameLayout.LayoutParams.WRAP_CONTENT
                ).apply { gravity = Gravity.BOTTOM; bottomMargin = 6 }
            })
            if (!placed) {
    ...
    private fun buildView(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
    ...
    @SuppressLint("ClickableViewAccessibility")
    private fun buildJoystick(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
            ...
            container.setOnTouchListener { _, e ->
                when (e.action) {
                    ...
                    MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                        thumb.x = cx - thumbSz / 2f
                        thumb.y = cy - thumbSz / 2f
                        if (isLeft) {
                            leftJoyX = 0
                            leftJoyY = 0
                        } else {
    ...
    private fun buildTouchpad(pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        ...
        val gridView = object : android.view.View(this) {
            val paint = Paint().apply {
                color = Color.argb(40, 156, 39, 176)
                strokeWidth = 1f
        ...
        container.addView(TextView(this).apply {
            text = "≋ TOUCHPAD"
            textSize = 9f
            setTextColor(Color.argb(150, 156, 39, 176))
            gravity = Gravity.CENTER
            layoutParams = FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.WRAP_CONTENT
    ...
    @SuppressLint("ClickableViewAccessibility")
    private fun buildKbKey(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
    ...
    @SuppressLint("ClickableViewAccessibility")
    private fun buildNormalBtn(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        container.background = android.graphics.drawable.GradientDrawable().apply {
        ...
        val iconIv = ImageView(this).apply {
            ...
            layoutParams = FrameLayout.LayoutParams(
                (pb.w * 0.45f).toInt(), (pb.h * 0.38f).toInt()
            ).apply {
                gravity = Gravity.CENTER_HORIZONTAL or Gravity.TOP
    ...
    private fun attachEditOverlay(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout) {
        ...
        val delBtn = TextView(this).apply {
            ...
            setOnClickListener {
                ...
                if (def.id == "right_joy" || def.id == "touchpad") {
                    rightJoyX = 0
                    rightJoyY = 0
                }
            }
    ...
    private fun sendReport() {
        try {
            ...
            // ✅ FIX: Send gas & brake as full 0-255 (unsigned) values
            ...
            // The order of bytes in this array must match the HID descriptor's order.
            // The order currently is: buttons[0], buttons[1], LX, LY, Z, Rz, Rx, Ry.
            // This means the last two bytes (Rx and Ry) correspond to Brake and Gas.
            val report = byteArrayOf(
                b1.toByte(), b2.toByte(),
                leftJoyX, leftJoyY,   // Left Stick
                rightJoyX, rightJoyY, // Right Stick / Touchpad
                brakeVal, gasVal      // Brake & Gas as unsigned triggers
            )
