...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val customButtons = listOf(
        // ── Default XML buttons ───────────────────────────────────────────────
        // ── byte1: standard face + shoulder + menu buttons (fixed order so
        //    PC/Android/games recognize them correctly — no more clashes) ──
        CustomBtn("btn_a",      "A",        R.drawable.btn_xbox_green,        0xFF3CFF6B.toInt(), byte1bit = 0),
        CustomBtn("btn_b",      "B",        R.drawable.btn_xbox_red,          0xFFFF4B4B.toInt(), byte1bit = 1),
        CustomBtn("btn_x",      "X",        R.drawable.btn_xbox_blue,         0xFF3DB9FF.toInt(), byte1bit = 2),
        CustomBtn("btn_y",      "Y",        R.drawable.btn_xbox_yellow,       0xFFFFD700.toInt(), byte1bit = 3),
        CustomBtn("gas",        "GAS / RT", R.drawable.ic_speed,              0xFF3CFF6B.toInt()),
        CustomBtn("brake",      "BRAKE / LT",R.drawable.ic_hand,              0xFFFF4B4B.toInt()),
        CustomBtn("gear_up",    "REVERSE",  R.drawable.expand_circle_up_24,   0xFFFF6D00.toInt(), byte2bit = 0),
        CustomBtn("gear_down",  "FRONT",    R.drawable.expand_circle_down_24, 0xFF00B4D8.toInt(), byte2bit = 1),
        ...
        // ── Warning group ─────────────────────────────────────────────────────
        CustomBtn("horn",       "HORN",     R.drawable.ic_btn_horn,           0xFFFFEB3B.toInt(), byte2bit = 2),
        ...
        // ── Drive controls ────────────────────────────────────────────────────
        CustomBtn("handbrake",  "H.BRAKE",  R.drawable.ic_btn_handbrake,      0xFFFF5722.toInt(), byte2bit = 3),
        ...
        CustomBtn("l3",         "L3",       R.drawable.ic_btn_l1,             0xFF303F9F.toInt(), byte3bit = 1),
        CustomBtn("r3",         "R3",       R.drawable.ic_btn_r1,             0xFF455A9C.toInt(), byte3bit = 2),
        CustomBtn("start",      "START",    R.drawable.ic_btn_start,          0xFF4CAF50.toInt(), byte1bit = 7),
        CustomBtn("select",     "SELECT",   R.drawable.ic_btn_select,         0xFF388E3C.toInt(), byte1bit = 6),
        // ── Look group ────────────────────────────────────────────────────────
        CustomBtn("look_left",  "LOOK ◀",   R.drawable.ic_btn_look_left,      0xFF795548.toInt(), byte2bit = 7),
        CustomBtn("look_right", "LOOK ▶",   R.drawable.ic_btn_look_right,     0xFF8D6E63.toInt(), byte3bit = 0),
        // ── Minimal group ─────────────────────────────────────────────────────
    ...
    private fun loadOrDefaultLayout() {
        ...
        val saved = prefs.getString("custom_v1", null)
        if (saved != null) {
            // User has a saved layout — load it
            loadCustomLayout()
        } else {
            // First launch — place all 12 defaults at XML positions
            placeDefaultButtons()
        }
    }
    ...
    private fun getBtnStyle(id: String): BtnStyle {
        return when {
            ...
            // ── Controller group: l1, r1, start, select — chrome style ─────────
            id == "l1" || id == "r1" || id == "start" || id == "select" -> BtnStyle(
