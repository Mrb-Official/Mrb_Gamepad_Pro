...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val customButtons = listOf(
        ...
        // Standard gamepad trigger duplicates. These use the exact same HID
        // analog channels as gas/brake, but expose clean RT/LT icons for Normal mode.
        CustomBtn("rt",         "RT",        R.drawable.ic_btn_rt,              0xFF3CFF6B.toInt()),
        CustomBtn("lt",         "LT",        R.drawable.ic_btn_lt,              0xFFFF4B4B.toInt()),
    ...
    /** Shown right when a Bluetooth device connects, so the user picks the
     *  layout that fits what they're about to play. */
    private fun showModePickerDialog() {
        // Netflix-style profile selection: clean, centered, no title/header.
        // A single instance is kept on screen so repeated Bluetooth callbacks
        // cannot stack multiple pickers.
        if (overlayFrame.findViewWithTag<View>("control_mode_picker") != null) return

        val d = resources.displayMetrics.density
        val picker = FrameLayout(this).apply {
            tag = "control_mode_picker"
        ...
        fun addProfile(iconRes: Int, label: String, accent: String, mode: String) {
            val cardSize = (184 * d).toInt()
            val card = LinearLayout(this).apply {
                orientation = LinearLayout.VERTICAL
                gravity = Gravity.CENTER
                layoutParams = LinearLayout.LayoutParams(cardSize, cardSize).apply {
                    setMargins((18 * d).toInt(), 0, (18 * d).toInt(), 0)
                }
                background = android.graphics.drawable.GradientDrawable().apply {
                    setColor(Color.parseColor("#101114"))
                    setStroke((2 * d).toInt(), Color.parseColor(accent))
                    cornerRadius = 22 * d
                }
                elevation = 14 * d
                isClickable = true
                isFocusable = true
                setOnClickListener {
                    animate().scaleX(0.94f).scaleY(0.94f).setDuration(90).withEndAction {
                        switchMode(mode)
                        picker.animate().alpha(0f).setDuration(140).withEndAction {
                            overlayFrame.removeView(picker)
                        }.start()
                    }.start()
                }
                setOnTouchListener { v, e ->
                    when (e.actionMasked) {
                        MotionEvent.ACTION_DOWN -> {
                            v.animate().scaleX(1.035f).scaleY(1.035f).setDuration(110).start()
                        }
        ...
        addProfile(R.drawable.ic_steering_wheel, "STEERING", "#8F9BFF", "wheel")
        addProfile(R.drawable.ic_gamepad_profile, "NORMAL", "#42C8FF", "normal")

        picker.addView(row)
        overlayFrame.addView(picker)
        picker.alpha = 0f
        picker.animate().alpha(1f).setDuration(180).start()
    }
    ...
    private fun placeNormalDefaults() {
        ...
        // ── Standard Xbox triggers: RT/LT, top corners ──
        val triggerW = (118 * d).toInt()
        val triggerH = (74 * d).toInt()
        placeDefault("lt", 16 * d, 78 * d, triggerW, triggerH)
        placeDefault("rt", sw - triggerW - 16 * d, 78 * d, triggerW, triggerH)
    ...
    private fun addCustomButton(def: CustomBtn, x: Float, y: Float, w: Int = -1, h: Int = -1) {
        ...
        val dw = if (w > 0) w else when {
            ...
            def.id == "tilt_tog"                           -> 130
            def.id == "kb_space"                           -> 160
            def.id == "gas" || def.id == "brake" || def.id == "rt" || def.id == "lt" -> 150
        ...
        val dh = if (h > 0) h else when {
            def.id == "left_joy" || def.id == "right_joy" -> 160
            def.id == "tilt_tog"                           -> 50
            def.id == "gas" || def.id == "brake" || def.id == "rt" || def.id == "lt" -> 180
    ...
    private fun buildView(def: CustomBtn, pb: PlacedCustomBtn,
        ...
        when {
            ...
            def.id == "gas" || def.id == "brake" || def.id == "rt" || def.id == "lt" -> buildGasBrakeBtn(def, pb, container, inEditMode)
    ...
    private fun buildGasBrakeBtn(def: CustomBtn, pb: PlacedCustomBtn,
                                  container: FrameLayout, inEditMode: Boolean) {
        val isGas   = def.id == "gas"
        val color   = def.pressColor  // green for gas, red for brake
        val d       = resources.displayMetrics.density

        // btn_normal_r12 style: dark fill, rounded 12dp, gray border
        ...
        // Label: 9sp bold, gray, gravity bottom, marginBottom=10dp — exact XML values
        val labelTv = TextView(this).apply {
            text = def.label  // "GAS / RT" or "BRAKE / LT"
            textSize = 9f
    ...
    private fun loadCustomLayout() {
        ...
        try {
            ...
            for (i in 0 until arr.length()) {
                ...
                val def = customButtons.find { it.id == id } ?: continue
                addCustomButton(def,
                    o.getDouble("x").toFloat(), o.getDouble("y").toFloat(),
                    o.optInt("w", -1), o.optInt("h", -1))
            }

            // Upgrade older Normal layouts without disturbing their saved positions.
            // If RT/LT were not present, add the new standard trigger controls.
            if (controlMode == "normal") {
                val d = resources.displayMetrics.density
                val sw = overlayFrame.width.toFloat()
                if (!placedCustomBtns.any { it.id == "lt" }) {
                    placeDefault("lt", 16 * d, 78 * d, (118 * d).toInt(), (74 * d).toInt())
                }
                if (!placedCustomBtns.any { it.id == "rt" }) {
                    placeDefault("rt", sw - 118 * d - 16 * d, 78 * d, (118 * d).toInt(), (74 * d).toInt())
                }
                saveCustomLayout()
            }
        } catch (_: Exception) {}
    ...
    private fun sendReport() {
        try {
            ...
            val brakeVal = if (customBtnStates["brake"] == true || customBtnStates["lt"] == true) 0xFF.toByte() else 0x00.toByte()
            val gasVal   = if (customBtnStates["gas"] == true || customBtnStates["rt"] == true) 0xFF.toByte() else 0x00.toByte()
