...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    // D-pad via hat switch (not bits anymore)
    ...
    // Default layout views — stored so we can drag them in edit mode
    private val defaultViews = mutableMapOf<String, View>()
    private val defaultPositions = mutableMapOf<String, Pair<Float, Float>>()
    ...
    private val customButtons = listOf(
        ...
        CustomBtn("touchpad",   "TOUCHPAD",R.drawable.ic_btn_custom,     0xFF9C27B0.toInt()),
        CustomBtn("tilt_tog",   "TILT",    R.drawable.ic_btn_custom,     0xFFFFD700.toInt()),
        // KB movement (axis only)
        ...
        // KB buttons (HID bits)
        CustomBtn("kb_q",       "Q",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte1bit = 2),
        CustomBtn("kb_e",       "E",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte1bit = 5),
        CustomBtn("kb_r",       "R",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte2bit = 0),
        CustomBtn("kb_f",       "F",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte2bit = 1),
        CustomBtn("kb_g",       "G",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte2bit = 4),
        CustomBtn("kb_space",   "SPACE",   R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte1bit = 3),
        CustomBtn("kb_shift",   "SHIFT",   R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte2bit = 3),
        CustomBtn("kb_ctrl",    "CTRL",    R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte2bit = 2),
        CustomBtn("kb_alt",     "ALT",     R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte2bit = 6),
        CustomBtn("kb_tab",     "TAB",     R.drawable.ic_btn_custom,     0xFF37474F.toInt(), byte2bit = 5),
        CustomBtn("kb_esc",     "ESC",     R.drawable.ic_btn_custom,     0xFF37474F.toInt(), byte2bit = 7),
        CustomBtn("kb_1",       "1",       R.drawable.ic_btn_custom,     0xFFF44336.toInt(), byte1bit = 0),
        CustomBtn("kb_2",       "2",       R.drawable.ic_btn_custom,     0xFFF44336.toInt(), byte1bit = 1),
        CustomBtn("kb_3",       "3",       R.drawable.ic_btn_custom,     0xFFF44336.toInt(), byte1bit = 4),
        CustomBtn("kb_4",       "4",       R.drawable.ic_btn_custom,     0xFFF44336.toInt(), byte1bit = 6),
        CustomBtn("kb_5",       "5",       R.drawable.ic_btn_custom,     0xFFF44336.toInt(), byte1bit = 7),
    ...
    private fun initUI() {
        ...
        // Store default views for edit mode dragging
        setupDefaultViews()
    ...
    private fun setupDefaultViews() {
        ...
        fun vib() { if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
            vibrator.vibrate(VibrationEffect.createOneShot(30, VibrationEffect.DEFAULT_AMPLITUDE)) }

        // Map view IDs to names for default layout
        val viewMap = mapOf(
            "lay_brake"     to R.id.lay_brake,
            "lay_gas"       to R.id.lay_gas,
            "lay_gear_up"   to R.id.lay_gear_up,
            "lay_gear_down" to R.id.lay_gear_down,
            "btn_a"         to R.id.btn_a,
            "btn_b"         to R.id.btn_b,
            "btn_x"         to R.id.btn_x,
            "btn_y"         to R.id.btn_y,
            "btn_dpad_up"   to R.id.btn_dpad_up,
            "btn_dpad_down" to R.id.btn_dpad_down,
            "btn_dpad_left" to R.id.btn_dpad_left,
            "btn_dpad_right" to R.id.btn_dpad_right
        )
        viewMap.forEach { (name, id) ->
    ...
    private fun showPremiumPopup() {
        ...
        val btnTry = FrameLayout(this).apply {
            background = android.graphics.drawable.GradientDrawable().apply {
                setColor(Color.TRANSPARENT); setStroke(1, Color.argb(80,255,255,255)); cornerRadius = 24f
            }
        ...
        listOf(
            ...
            row.addView(TextView(this).apply { text = label; textSize = 11f; setTextColor(Color.WHITE) })
    ...
    private fun toggleEditMode() {
        ...
        if (editMode) {
            showEditBar()
            // Make default views draggable
            enableDefaultViewDragging(true)
            // Rebuild custom buttons in edit mode
            placedCustomBtns.toList().forEach { pb ->
                ...
                v.removeAllViews(); buildView(def, pb, v, inEditMode = true)
            }
        } else {
            hideEditBar()
            enableDefaultViewDragging(false)
            placedCustomBtns.toList().forEach { pb ->
                val v   = placedCustomViews[pb.id] ?: return@forEach
                val def = customButtons.find { it.id == pb.id } ?: return@forEach
                v.removeAllViews(); buildView(def, pb, v, inEditMode = false)
            }
        }
    }

    @SuppressLint("ClickableViewAccessibility")
    private fun enableDefaultViewDragging(enable: Boolean) {
        defaultViews.forEach { (name, view) ->
            if (enable) {
                // Show yellow border
                view.foreground = android.graphics.drawable.GradientDrawable().apply {
                ...
                view.setOnTouchListener { v, e ->
                    when (e.action) {
                        ...
                        MotionEvent.ACTION_MOVE -> {
                            ...
                            defaultPositions[name] = Pair(v.x, v.y); true
                        }
                ...
                when (name) {
                    "lay_brake"      -> setupTouch(R.id.lay_brake,     R.drawable.btn_normal_r12, R.drawable.btn_press_red,    R.id.ic_brake,     0xFFFF4B4B.toInt()) { brakeOn = it }
                    "lay_gas"        -> setupTouch(R.id.lay_gas,       R.drawable.btn_normal_r12, R.drawable.btn_press_green,  R.id.ic_gas,       0xFF3CFF6B.toInt()) { gasOn = it }
                    "lay_gear_up"    -> setupTouch(R.id.lay_gear_up,   R.drawable.btn_gear_normal, R.drawable.btn_press_orange, R.id.ic_gear_up,   0xFFFF6D00.toInt()) { gearUp = it }
                    "lay_gear_down"  -> setupTouch(R.id.lay_gear_down, R.drawable.btn_gear_normal, R.drawable.btn_press_blue,   R.id.ic_gear_down, 0xFF00B4D8.toInt()) { gearDown = it }
                    "btn_a"          -> setupTouch(R.id.btn_a, R.drawable.btn_xbox_green,  R.drawable.btn_xbox_green_press,  null, 0) { btnA = it }
                    "btn_b"          -> setupTouch(R.id.btn_b, R.drawable.btn_xbox_red,    R.drawable.btn_xbox_red_press,    null, 0) { btnB = it }
                    "btn_x"          -> setupTouch(R.id.btn_x, R.drawable.btn_xbox_blue,   R.drawable.btn_xbox_blue_press,   null, 0) { btnX = it }
                    "btn_y"          -> setupTouch(R.id.btn_y, R.drawable.btn_xbox_yellow, R.drawable.btn_xbox_yellow_press, null, 0) { btnY = it }
                    "btn_dpad_up"    -> setupTouch(R.id.btn_dpad_up,    R.drawable.btn_normal_r12, R.drawable.btn_press_white, null, 0) { dpadUp    = it }
                    "btn_dpad_down"  -> setupTouch(R.id.btn_dpad_down,  R.drawable.btn_normal_r12, R.drawable.btn_press_white, null, 0) { dpadDown  = it }
                    "btn_dpad_left"  -> setupTouch(R.id.btn_dpad_left,  R.drawable.btn_normal_r12, R.drawable.btn_press_white, null, 0) { dpadLeft  = it }
                    "btn_dpad_right" -> setupTouch(R.id.btn_dpad_right, R.drawable.btn_normal_r12, R.drawable.btn_press_white, null, 0) { dpadRight = it }
                }
            }
        }
    }
    ...
    private fun showEditBar() {
        ...
        bar.addView(iconBarBtn(R.drawable.add_circle_24, "ADD",   "#00C853") { showPicker() })
        ...
        bar.addView(View(this).apply { layoutParams = LinearLayout.LayoutParams(0, 1, 1f) })
        bar.addView(iconBarBtn(R.drawable.save_24, "SAVE", "#2196F3") { saveCustomLayout(); toggleEditMode() })
        bar.addView(iconBarBtn(R.drawable.auto_delete_24, "RESET", "#F44336") { confirmReset() })
        ...
        for (btn in customButtons) {
            ...
            if (!placed) {
                chip.setOnClickListener {
                    addCustomButton(btn,
                        (overlayFrame.width  / 2 - 60).toFloat(),
                        (overlayFrame.height / 2 - 60).toFloat())
                }
            }
            grid.addView(chip)
        }
    ...
    private fun buildJoystick(def: CustomBtn, pb: PlacedCustomBtn,
        ...
        val hexColor = if (isLeft) "#00BCD4" else "#4CAF50"
        val size     = pb.w
        ...
        val thumb = FrameLayout(this).apply {
            background = android.graphics.drawable.GradientDrawable().apply {
                shape = android.graphics.drawable.GradientDrawable.OVAL
                setColor(Color.argb(136, Color.red(parsed), Color.green(parsed), Color.blue(parsed)))
                setStroke(2, parsed)
            }
    ...
    private fun buildTouchpad(pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        container.background = android.graphics.drawable.GradientDrawable().apply {
            setColor(Color.argb(30, 156, 39, 176))
            setStroke(2, Color.parseColor("#9C27B0"))
            cornerRadius = 16f
        }
        val gridView = object : android.view.View(this) {
            val paint = Paint().apply {
                color = Color.argb(40, 156, 39, 176); strokeWidth = 1f; style = Paint.Style.STROKE
            }
            override fun onDraw(canvas: Canvas) {
                val cw = width / 4f; val ch = height / 3f
                for (i in 1..3) canvas.drawLine(i*cw, 0f, i*cw, height.toFloat(), paint)
                for (i in 1..2) canvas.drawLine(0f, i*ch, width.toFloat(), i*ch, paint)
            }
        }
        ...
        container.addView(TextView(this).apply {
            text = "≋ TOUCHPAD"; textSize = 9f
            setTextColor(Color.argb(150, 156, 39, 176)); gravity = Gravity.CENTER
            layoutParams = FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT,
                FrameLayout.LayoutParams.WRAP_CONTENT).apply {
                gravity = Gravity.TOP or Gravity.CENTER_HORIZONTAL; topMargin = 4
            }
        })
        val dot = View(this).apply {
            ...
            layoutParams = FrameLayout.LayoutParams(14, 14).apply { gravity = Gravity.CENTER }
            alpha = 0.4f
        }
        container.addView(dot)

        if (!inEditMode) {
            var lastX = 0f; var lastY = 0f; var firstTouch = true
            val sensitivity = 2.0f
            container.setOnTouchListener { _, e ->
                when (e.action) {
                    MotionEvent.ACTION_DOWN -> {
                        lastX = e.x; lastY = e.y; firstTouch = true
                        dot.x = e.x - 7f; dot.y = e.y - 7f; dot.alpha = 1f; true
                    }
                    MotionEvent.ACTION_MOVE -> {
                        if (!firstTouch) {
                            val dx = (e.x - lastX) * sensitivity
                            val dy = (e.y - lastY) * sensitivity
                            rightJoyX = (rightJoyX + dx).toInt().coerceIn(-127, 127).toByte()
                            rightJoyY = (rightJoyY + dy).toInt().coerceIn(-127, 127).toByte()
                        }
                        firstTouch = false; lastX = e.x; lastY = e.y
                        dot.x = e.x - 7f; dot.y = e.y - 7f; true
                    }
                    MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                        rightJoyX = 0; rightJoyY = 0; dot.alpha = 0.4f
                        dot.x = container.width / 2f - 7f
                        dot.y = container.height / 2f - 7f; true
                    }
                    else -> false
                }
            }
    ...
    private fun buildTiltToggle(pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        ...
        container.addView(TextView(this).apply {
            text = if (tiltEnabled) "🔄 TILT ON" else "🔄 TILT OFF"; textSize = 11f
        ...
        if (!inEditMode) {
            container.setOnClickListener {
                ...
                container.removeAllViews(); buildTiltToggle(pb, container, false)
    ...
    private fun buildKbKey(def: CustomBtn, pb: PlacedCustomBtn,
                            container: FrameLayout, inEditMode: Boolean) {
        val isMovement = def.id in setOf("kb_w","kb_a","kb_s","kb_d","kb_up","kb_down","kb_left","kb_right")
        container.background = android.graphics.drawable.GradientDrawable().apply {
            setColor(if (isMovement) Color.argb(80,33,150,243) else Color.argb(60,200,200,200))
            setStroke(2, if (isMovement) Color.parseColor("#2196F3") else Color.argb(120,255,255,255))
            cornerRadius = 12f
        }
        ...
        if (!inEditMode) {
            ...
            container.setOnTouchListener { _, e ->
                ...
                if (pressed || released) {
                    ...
                    container.background = android.graphics.drawable.GradientDrawable().apply {
                        setColor(if (pressed)
                            (if (isMovement) Color.parseColor("#992196F3") else Color.argb(160,255,255,255))
                        else
                            (if (isMovement) Color.argb(80,33,150,243) else Color.argb(60,200,200,200)))
                        setStroke(2, if (isMovement) Color.parseColor("#2196F3") else Color.argb(120,255,255,255))
                        cornerRadius = 12f
                    }
                    tv.setTextColor(if (pressed && !isMovement) Color.parseColor("#111111") else Color.WHITE)
        ...
        resizeBtn.setOnTouchListener { _, e ->
            when (e.action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    val nw = (rsW + d).coerceIn(40, 320)
                    val nh = (rsH + d / 2).coerceIn(40, 320)
    ...
    private fun sendReport() {
        ...
        // Left axis = WASD or left joystick or tilt
        val finalLeftX: Byte = when {
            dpadLeft  -> (-127).toByte()
            dpadRight -> 127.toByte()
            leftJoyX.toInt() != 0 -> leftJoyX
            else -> tiltByte
        }
        val finalLeftY: Byte = when {
            dpadUp   -> (-127).toByte()
            dpadDown -> 127.toByte()
            else     -> leftJoyY
        }

        // Right axis = right joystick or touchpad (Z, Rz)
        val finalRightX = rightJoyX
        val finalRightY = rightJoyY

        // Hat switch for d-pad cross (+)
        // Default buttons dpad = hat switch
        // But if WASD used = axis (above), hat = center
        val hatVal: Byte = when {
            dpadUp    && !dpadLeft && !dpadRight -> 0  // North
            dpadUp    && dpadRight               -> 1  // NE
            dpadRight && !dpadUp   && !dpadDown  -> 2  // East
            dpadDown  && dpadRight               -> 3  // SE
            dpadDown  && !dpadLeft && !dpadRight -> 4  // South
            dpadDown  && dpadLeft                -> 5  // SW
            dpadLeft  && !dpadUp   && !dpadDown  -> 6  // West
            dpadUp    && dpadLeft                -> 7  // NW
            else                                 -> 8  // Center (released)
        }
