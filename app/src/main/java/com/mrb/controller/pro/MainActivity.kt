...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private val customButtons = listOf(
        ...
        CustomBtn("handbrake",  "H.BRAKE", R.drawable.ic_btn_handbrake,  0xFFFF5722.toInt()),
        CustomBtn("camera",     "CAM",     R.drawable.ic_btn_camera,     0xFF9C27B0.toInt()),
        CustomBtn("nitro",      "NITRO",   R.drawable.ic_btn_nitro,      0xFF00BCD4.toInt()),
        CustomBtn("lights",     "LIGHTS",  R.drawable.ic_btn_lights,     0xFFFFC107.toInt()),
        CustomBtn("look_left",  "LOOK<",   R.drawable.ic_btn_look_left,  0xFF795548.toInt()),
        CustomBtn("look_right", "LOOK>",   R.drawable.ic_btn_look_right, 0xFF8D6E63.toInt()),
        CustomBtn("l1",         "L1",      R.drawable.ic_btn_l1,         0xFF3F51B5.toInt()),
        CustomBtn("r1",         "R1",      R.drawable.ic_btn_r1,         0xFF5C6BC0.toInt()),
        CustomBtn("start",      "START",   R.drawable.ic_btn_start,      0xFF4CAF50.toInt()),
        CustomBtn("select",     "SELECT",  R.drawable.ic_btn_select,     0xFF388E3C.toInt()),
                    ...
                    v.post {
                        ...
                        val badge = FrameLayout(this).apply {
                            ...
                            setOnClickListener {
                                ...
                                Toast.makeText(context, "Default Button Removed", Toast.LENGTH_SHORT).show()
                            }
                        }
    ...
    private fun buildKbKey(def: CustomBtn, pb: PlacedCustomBtn,
        ...
        if (!inEditMode) {
            ...
            container.setOnTouchListener { _, e ->
                ...
                if (pressed || released) {
                    // 🔥 MAP WASD AND ARROWS DIRECTLY TO D-PAD BUTTONS
                    when (def.id) {
    ...
    private fun sendReport() {
        try {
            ...
            // 🔥 THE ULTIMATE 64-BUTTON FIX 🔥
            // We use a 64-bit Long to store up to 64 buttons simultaneously
            var btnBits = 0L
            
            // Standard Buttons (Bits 0 to 9)
            if (btnA)     btnBits = btnBits or (1L shl 0)
            if (btnB)     btnBits = btnBits or (1L shl 1)
            if (btnX)     btnBits = btnBits or (1L shl 2)
            if (btnY)     btnBits = btnBits or (1L shl 3)
            if (gearDown) btnBits = btnBits or (1L shl 4) // L1
            if (gearUp)   btnBits = btnBits or (1L shl 5) // R1
            
            // D-Pad and WASD Keys (Bits 6 to 9)
            if (dpadUp)   btnBits = btnBits or (1L shl 6)
            if (dpadDown) btnBits = btnBits or (1L shl 7)
            if (dpadLeft) btnBits = btnBits or (1L shl 8)
            if (dpadRight)btnBits = btnBits or (1L shl 9)

            // ALL Custom and Keyboard Buttons (Bits 10 to 60)
            var customOffset = 10
            for (def in customButtons) {
                // Ignore joysticks/tilt and WASD (handled above)
                if (def.id in listOf("left_joy", "right_joy", "touchpad", "tilt_tog", "kb_w", "kb_a", "kb_s", "kb_d", "kb_up", "kb_down", "kb_left", "kb_right")) continue
                
                if (customBtnStates[def.id] == true) {
                    btnBits = btnBits or (1L shl customOffset)
                }
                customOffset++
            }
            
            // Convert the 64-bit Long into 8 Bytes
            val b0 = (btnBits and 0xFF).toByte()
            val b1 = ((btnBits ushr 8) and 0xFF).toByte()
            val b2 = ((btnBits ushr 16) and 0xFF).toByte()
            val b3 = ((btnBits ushr 24) and 0xFF).toByte()
            val b4 = ((btnBits ushr 32) and 0xFF).toByte()
            val b5 = ((btnBits ushr 40) and 0xFF).toByte()
            val b6 = ((btnBits ushr 48) and 0xFF).toByte()
            val b7 = ((btnBits ushr 56) and 0xFF).toByte()
            ...
            // 🔥 Sending exactly 14 Bytes to match the New HID Descriptor
            hid.sendReport(device, 1,
                byteArrayOf(
                    b0, b1, b2, b3, b4, b5, b6, b7,     // 8 Bytes of Buttons
                    finalLeftX, finalLeftY, rightJoyX, rightJoyY, // 4 Bytes of Axes
                    brake, gas // 2 Bytes of Triggers
                )
            )
        } catch (e: Exception) {
            e.printStackTrace()
