...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    private fun initUI() {
        ...
        overlayFrame = object : FrameLayout(this) {
            override fun onInterceptTouchEvent(e: MotionEvent) = false
            override fun dispatchTouchEvent(e: MotionEvent): Boolean {
                // Check if any child contains this touch point
                ...
                // No child hit — let the event fall through to XML layout below
                return false
            }
        }.apply {
    ...
    private fun buildKbKey(def: CustomBtn, pb: PlacedCustomBtn,
        ...
        if (!inEditMode) {
            ...
            container.setOnTouchListener { _, e ->
                ...
                if (pressed || released) {
                    when (def.id) {
                        "kb_up"    -> dpadUp    = pressed
                        "kb_left"  -> dpadLeft  = pressed
                        "kb_down"  -> dpadDown  = pressed
                        "kb_right" -> dpadRight = pressed
                    }
