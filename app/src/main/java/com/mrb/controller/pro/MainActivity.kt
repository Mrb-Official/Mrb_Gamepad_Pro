...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    // ── Button definitions ────────────────────────────────────────────────────
    // byte1bit / byte2bit = HID button bit mapping
    // kb_ buttons with byte1bit/byte2bit also send button presses
    // kb_w/a/s/d/up/down/left/right = axis movement only (no button bit)
    ...
    private val customButtons = listOf(
        ...
        // Keyboard - movement only (axis, no button bit)
        ...
        // Keyboard - button press (mapped to HID buttons)
        CustomBtn("kb_q",        "Q",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte1bit = 2),  // same as horn
        CustomBtn("kb_e",        "E",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte1bit = 5),  // same as cam
        CustomBtn("kb_r",        "R",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte2bit = 0),  // nitro
        CustomBtn("kb_f",        "F",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte2bit = 1),  // lights
        CustomBtn("kb_g",        "G",       R.drawable.ic_btn_custom,     0xFF546E7A.toInt(), byte2bit = 4),  // look left
        CustomBtn("kb_space",    "SPACE",   R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte1bit = 3),  // handbrake
        CustomBtn("kb_shift",    "SHIFT",   R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte2bit = 3),  // r1
        CustomBtn("kb_ctrl",     "CTRL",    R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte2bit = 2),  // l1
        CustomBtn("kb_alt",      "ALT",     R.drawable.ic_btn_custom,     0xFF455A64.toInt(), byte2bit = 6),  // select
        CustomBtn("kb_tab",      "TAB",     R.drawable.ic_btn_custom,     0xFF37474F.toInt(), byte2bit = 5),  // start
        CustomBtn("kb_esc",      "ESC",     R.drawable.ic_btn_custom,     0xFF37474F.toInt(), byte2bit = 7),  // look right
    ...
    private fun initUI() {
        ...
        setupTouch(R.id.lay_brake,     R.drawable.btn_normal_r12,    R.drawable.btn_press_red,     null,     0xFFFF4B4B.toInt()) { brakeOn = it }
        setupTouch(R.id.lay_gas,       R.drawable.btn_normal_r12,    R.drawable.btn_press_green,   null,       0xFF3CFF6B.toInt()) { gasOn = it }
        setupTouch(R.id.lay_gear_up,   R.drawable.btn_gear_normal,   R.drawable.btn_press_orange,  null,   0xFFFF6D00.toInt()) { gearUp = it }
        setupTouch(R.id.lay_gear_down, R.drawable.btn_gear_normal,   R.drawable.btn_press_blue,    null, 0xFF00B4D8.toInt()) { gearDown = it }
    ...
    private fun isPremium(): Boolean {
        val prefs = getSharedPreferences("mrb_premium", MODE_PRIVATE)
    ...
    private fun onCrownClick() {
        val existingPopup = overlayFrame.findViewWithTag<View>("premium_popup")
        if (existingPopup != null) {
            existingPopup.animate().alpha(0f).translationY(80f).setDuration(250)
                .withEndAction { overlayFrame.removeView(existingPopup) }
                .start()
            return
        }
    ...
    private fun dismissPopup() {
        ...
        v.animate().alpha(0f).translationY(80f).setDuration(250)
            .withEndAction { overlayFrame.removeView(v) }
            .start()
    }
    ...
    private fun showPremiumPopup() {
        ...
        val desc = TextView(this).apply {
            text = "Unlock Custom Button Layout\nAdd any buttons anywhere on screen."
            textSize = 14f
        ...
        val btnWatch = FrameLayout(this).apply {
            ...
            val tv = TextView(context).apply {
                text = "▶ Watch Ad for 2 Hours Access"
                textSize = 14f
            ...
            setOnClickListener {
                if (rewardedAd != null) {
                    rewardedAd?.show(this@MainActivity) { _ ->
                        val expiry = System.currentTimeMillis() + (2 * 60 * 60 * 1000)
                        getSharedPreferences("mrb_premium", MODE_PRIVATE).edit()
                            .putLong("premium_expiry", expiry).apply()
                        dismissPopup()
                        crownView?.let { c -> updateCrownGlow(c) }
                        Toast.makeText(this@MainActivity, "Premium Unlocked for 2 Hours!", Toast.LENGTH_LONG).show()
                        toggleEditMode()
                    }
                    rewardedAd = null
                    loadRewardedAd()
                } else {
                    Toast.makeText(this@MainActivity, "Ad loading... Try again in a few seconds.", Toast.LENGTH_SHORT).show()
                    loadRewardedAd()
                }
            }
        }
        card.addView(btnWatch)
    ...
    private fun showEditBar() {
        ...
        val space1 = View(this).apply { layoutParams = LinearLayout.LayoutParams(0, 1, 1f) }
        ...
        val btnReset = iconBarBtn(R.drawable.auto_delete_24, "RESET", "#F44336") {
            confirmReset()
        }
    ...
    private fun createPickerChip(label: String, iconRes: Int, placed: Boolean, pressColor: Int, onClick: () -> Unit): FrameLayout {
        val chip = FrameLayout(this).apply {
            ...
            val bg = if (placed) Color.parseColor("#1A1A1A")
    ...
    private fun buildJoystick(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        ...
        val colorHex = if (isLeft) "#00BCD4" else "#4CAF50"
        val color = Color.parseColor(colorHex)
        val colorBg = Color.parseColor(colorHex.replace("#", "#88"))
        ...
        val thumb = FrameLayout(this).apply {
            ...
            layoutParams = FrameLayout.LayoutParams(thumbSz, thumbSz).apply {
                gravity = Gravity.CENTER
            }
        }
        val tv = TextView(this).apply {
    ...
    private fun buildTiltToggle(pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        val color = if (tiltEnabled) Color.parseColor("#FFD700") else Color.parseColor("#666666")
        val colorBg = if (tiltEnabled) Color.parseColor("#44FFD700") else Color.parseColor("#22888888")
    ...
    private fun buildNormalBtn(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout, inEditMode: Boolean) {
        ...
        val iv = ImageView(this).apply {
            ...
            layoutParams = FrameLayout.LayoutParams((pb.w * 0.45f).toInt(), (pb.h * 0.38f).toInt()).apply {
                gravity = Gravity.CENTER_HORIZONTAL or Gravity.TOP
            ...
            container.setOnTouchListener { _, e ->
                when (e.actionMasked) {
                    MotionEvent.ACTION_DOWN, MotionEvent.ACTION_POINTER_DOWN -> {
                        ...
                        container.background = android.graphics.drawable.GradientDrawable().apply {
                            setColor(Color.argb(60, Color.red(def.pressColor), Color.green(def.pressColor), Color.blue(def.pressColor)))
                            setStroke(2, Color.parseColor("#444444"))
                            cornerRadius = 16f
                        }
                        iv.setColorFilter(def.pressColor)
                        tv.setTextColor(def.pressColor)
    ...
    private fun attachEditOverlay(def: CustomBtn, pb: PlacedCustomBtn, container: FrameLayout) {
        ...
        val btnResize = TextView(this).apply {
            text = "⤡"
            textSize = 12f
            setTextColor(Color.WHITE)
            gravity = Gravity.CENTER
        ...
        btnResize.setOnTouchListener { _, e ->
            when (e.action) {
                MotionEvent.ACTION_DOWN -> {
                    isResizing = true
                    rsStartX = e.rawX
                    rsStartW = container.width
                    rsStartH = container.height
                    true
                }
                MotionEvent.ACTION_MOVE -> {
                    val delta = (e.rawX - rsStartX).toInt()
                    val nw = (rsStartW + delta).coerceIn(40, 600)
                    val nh = (rsStartH + delta / 2).coerceIn(40, 600)
    ...
    private fun saveCustomLayout() {
        ...
        prefs.edit().putString("custom_v1", arr.toString()).apply()
        Toast.makeText(this, "✅ Layout saved!", Toast.LENGTH_SHORT).show()
    }

    private fun loadCustomLayout() {
        val prefs = getSharedPreferences("mrb_custom", MODE_PRIVATE)
        val json = prefs.getString("custom_v1", null) ?: return
        try {
            ...
            for (i in 0 until arr.length()) {
                ...
                addCustomButton(def, obj.getDouble("x").toFloat(), obj.getDouble("y").toFloat(),
                    obj.optInt("w", -1), obj.optInt("h", -1))
            }
        } catch (_: Exception) {}
    }
    ...
    private fun setupTouch(id: Int, normalRes: Int, pressRes: Int, iconId: Int?, pressIconColor: Int, onPress: (Boolean) -> Unit) {
        ...
        val icon = if (iconId != null) findViewById<ImageView>(iconId) else null
        val vibrator = getSystemService(Context.VIBRATOR_SERVICE) as Vibrator
    ...
    private fun sendReport() {
        try {
            ...
            // THE FIX: Applying the exact correct variables to the correct axes to prevent overlap
